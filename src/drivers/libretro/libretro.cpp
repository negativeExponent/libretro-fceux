...
static void check_game_genie_variable(void) {
	...
	if ((GameInfo && (GameInfo->type == GIT_CART)) &&
	    (GameInfo->mappernum != 105) && /* Nintendo World Championship cart (Mapper 105)*/
...
DUMMY(FCEUMOV_PreLoad)
bool FCEUMOV_PostLoad() { return true; }
...
bool autoMovieBackup = false;
bool freshMovie = false;
 ...
 */
void FCEUD_SoundToggle(void) { FCEUI_SetSoundVolume(100); }
